...
def retrieval_qa_pipline(device_type, use_history, model_number, promptTemplate_type="llama"):
    """
    ...
    model_id = globals()["MODEL_ID_" + model_number]
    model_basename = globals()["MODEL_BASENAME_" + model_number]
...
def main(device_type, show_sources, use_history, model_type, save_qa, inputprompt, saveoutput = False, roundtable_participant= '0'):
    """
    ...
    qa = retrieval_qa_pipline(device_type, use_history, roundtable_participant, promptTemplate_type=model_type)
    # Interactive questions and answers

    with open (inputprompt, 'r') as f:
        query = f.read()
    ...
    model_basename = globals()["MODEL_BASENAME_" + roundtable_participant]

    #if saving is wanted
    if saveoutput is not None:
        with open(saveoutput, 'a') as file:
            file.write("\n\n>>> " + beautify_pipe(model_basename) +": ")
            file.write(answer)
