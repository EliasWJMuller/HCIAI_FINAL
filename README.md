# Too Many Cooks? - Evaluating LLM Output Qulity and Required User Vigilance in Multi-Agent Interaction

This repository contains the implementation of the final project for the HCI AI course though by Dan Russell and is co-authered by Niklas Schmidt, Diyar Taskiran and Elias Müller.

Find below the official documentation of the LocalGPT project.

## Acknowledgements

This repository contains primarily the code base of the LocalGPT project. We modified the following files in accordance to the LICENSE: 
```

   ├── constants.py                # Model Definitions
   ├── run_hcaigpt.py              # Implementation of a roundtable setting
   ├── prompt_template_utils.py    # Project Specific System Prompt    
   ├── runhciai_handler.ipynb      # File facilitating user interacation and passing the required inputs to the respecive models, plus logging      
   └── README.md  
```               

## Usage

This project focuses on the HCIAI aspect of a multi-agent chat bots and thus does not aim to proviode a production ready/ distriubutable code base.
For testing purposes the `runhciai_handler.py` can be used to instantiate the roundtable with the LLM "llama-2-7b-chat.Q4_K_M.gguf" and "mistral-7b-instruct-v0.1.Q8_0.gguf". Note, instantiating these models locally requires significant memory and downloads the models from Huggingface. 
The desired input-promt can be provided in the outputanswers.py file - or any other txt file (changes to handler input required). The handler itself runs a subprocess in the CLI to prompt the models. While it is an inefficient solution requiring significant CPU load each iteration, it serves the purpose of studying desired the HCIAI aspects well.
